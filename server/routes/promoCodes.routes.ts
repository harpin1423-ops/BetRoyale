 ...
 */
function formatDateInColombia(value: Date): string {
 ...
 */
function normalizarFechaCupon(value: unknown): string | null {
 ...
 */
function obtenerAhoraColombiaMysql(): string {
...
router.get("/", authenticateToken, requireAdmin, async (_req, res) => {
  try {
    ...
    const [codigos] = await pool.query(
      `SELECT id, code, discount_percentage, max_uses, current_uses,
