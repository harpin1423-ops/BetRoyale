...
export async function startServer(): Promise<void> {
  ...
  app.get("/api/debug/status", async (_req, res) => {
    try {
      const [c]: any = await pool.query("SELECT COUNT(*) as total FROM countries");
      const [l]: any = await pool.query("SELECT COUNT(*) as total FROM leagues");
      const [t]: any = await pool.query("SELECT COUNT(*) as count FROM teams");
      const [u]: any = await pool.query("SELECT COUNT(*) as total FROM users");
      const [p]: any = await pool.query("SELECT COUNT(*) as total FROM picks");
  ...
  app.listen(PORT, "0.0.0.0", () => {
    ...
    // Iniciamos las tareas programadas (Resultados automáticos cada hora)
    initCronJobs();
  });
}
