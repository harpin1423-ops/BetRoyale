 ...
 */
let tasaCache: { valor: number; timestamp: number } | null = null;
 ...
 */
export async function obtenerTasaCambio(): Promise<number> {
...
/**
 * Parámetros necesarios para activar o renovar una suscripción de usuario.
 */
interface ParametrosActivacion {
  /** ID del usuario en la base de datos */
  userId: string;
  /** Slug del plan (ej: 'cuota_2', 'cuota_3') */
  planId: string;
  /** Período de la suscripción: 'mensual' | 'trimestral' | 'semestral' | 'anual' */
  period: string;
  /** Monto pagado en COP */
  amount: number | null | undefined;
  /** Moneda del pago (ej: 'COP') */
 ...
 */
export async function activarSuscripcion(params: ParametrosActivacion): Promise<void> {
  ...
  const [subsExistentes]: any = await pool.query(
  ...
  if (subsExistentes.length > 0) {
    ...
    await pool.query(
      `UPDATE user_subscriptions 
       ...
       WHERE id = ?`,
      [expiracionFormateada, amount, paymentMethod, period, currency, montoUSD, subsExistentes[0].id]
    );
  } else {
    ...
    await pool.query(
      `INSERT INTO user_subscriptions 
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [userId, planId, expiracionFormateada, amount, paymentMethod, period, currency, montoUSD]
    );
  }
  ...
  const [todasSubs]: any = await pool.query(
    "SELECT MAX(expires_at) as max_expiracion FROM user_subscriptions WHERE user_id = ?",
    [userId]
  );
  ...
  await pool.query(
    `UPDATE users 
     ...
     WHERE id = ?`,
    [maxExpiracionFormateada, planId, userId]
  );
 ...
 */
function obtenerMensajeMercadoPago(payload: any): string {
 ...
 */
function construirHeadersMercadoPago(): Record<string, string> {
 ...
 */
export async function createMercadoPagoPreference(body: Record<string, unknown>): Promise<any> {
