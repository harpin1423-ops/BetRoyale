...
// Estado público que entiende el frontend para mostrar la situación del acceso.
export type EstadoInviteVip = "available" | "used" | "expired";
...
interface ParametrosInviteVip {
  ...
  name: string;
  /** Fuerza la creación de un link nuevo aunque exista uno vigente en caché. */
...
export interface ResultadoInviteVip {
  ...
  /** Estado actual del link privado del usuario. */
  status: EstadoInviteVip;
  /** Fecha en que Telegram confirmó el ingreso del usuario. */
  used_at?: string | null;
  /** Usuario de Telegram que ingresó con el link, si Telegram lo reportó. */
 ...
 */
function construirInviteUsado(name: string, fila: any): ResultadoInviteVip {
 ...
 */
export async function marcarInviteVipComoUsado(
  inviteLink: string,
...
export async function obtenerOCrearInviteVip(
  ...
  const [existentes]: any = await pool.query(
    `SELECT invite_link, expires_at, used_at, revoked_at, telegram_username
     FROM telegram_user_invites
     WHERE user_id = ?
       AND plan_id = ?
       AND channel_id = ?
     LIMIT 1`,
    [params.userId, params.planId, channelId]
  );
