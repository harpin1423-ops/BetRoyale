...
const PROVIDER_ORDER = ["gemini", "groq", "deepseek", "openai"];
...
function buildPromptContext(pickData) {
...
export async function generatePickAnalysis(pickData) {
    ...
    if (requestedProvider !== "auto") {
        if (!isProviderAvailable(requestedProvider)) {
            throw new Error(`El proveedor "${getProviderLabel(requestedProvider)}" no tiene API key configurada en el servidor.`);
        }
    ...
    if (availableProviders.length === 0) {
        throw new Error("No hay proveedores de IA configurados. Agrega al menos una de estas variables en el .env: GEMINI_API_KEY, GROQ_API_KEY, OPENAI_API_KEY");
    }
    const errors = [];
    ...
    throw new Error(`Todos los proveedores de IA alcanzaron su límite de uso:\n${errors.join("\n")}\n\nEspera unos minutos o configura más proveedores.`);
}
