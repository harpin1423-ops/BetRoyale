 ...
 */
function normalizarSetting(value) {
 ...
 */
export async function obtenerSetting(key, fallback = "") {
 ...
 */
export async function guardarSetting(key, value) {
    ...
    await pool.query(`INSERT INTO app_settings (setting_key, setting_value)
     VALUES (?, ?)
     ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value)`, [key, valorNormalizado]);
 ...
 */
export async function obtenerTelegramFullConfig() {
    // Leemos el Channel ID desde DB y usamos .env como respaldo local/producción.
    const telegramChannelId = await obtenerSetting(SETTINGS_KEYS.telegramFullChannelId, env.TELEGRAM_FULL_CHANNEL_ID);
    // Leemos el invite link desde DB y usamos .env como respaldo.
    const telegramInviteLink = await obtenerSetting(SETTINGS_KEYS.telegramFullInviteLink, env.TELEGRAM_FULL_INVITE_LINK);
