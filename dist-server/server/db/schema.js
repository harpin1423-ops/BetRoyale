...
export async function initDB() {
    ...
    `);
        ...
        for (const query of columnasUsers) {
            await conexion.query(query).catch((e) => {
                if (e.code !== "ER_DUP_FIELDNAME")
                    console.warn("[DB] Migración usuarios:", e.message);
            });
        }
        // ── 6. Tabla: picks ──────────────────────────────────────────────────────
    ...
    `);
        ...
        for (const query of columnasSubscriptions) {
            await conexion.query(query).catch((e) => {
                if (e.code !== "ER_DUP_FIELDNAME")
                    console.warn("[DB] Migración subs:", e.message);
            });
        }
    ...
    `);
        ...
        for (const { pais, ligas } of LIGAS_INICIALES) {
            try {
                ...
                const [filas] = await conexion.query("SELECT id FROM countries WHERE name = ?", [pais]);
                ...
                if (codisoFlag) {
                    await conexion.query("UPDATE countries SET flag = ? WHERE id = ? AND (flag IS NULL OR flag = '')", [codisoFlag, idPais]);
                }
        ...
        const [admins] = await conexion.query("SELECT * FROM users WHERE email = 'admin@betroyale.club'");
        if (admins.length === 0) {
            // Solo creamos el admin si no existe (no sobreescribimos contraseñas)
            const hashAdmin = await bcrypt.hash("admin123", 10);
            await conexion.query("INSERT INTO users (email, password_hash, role) VALUES ('admin@betroyale.club', ?, 'admin')", [hashAdmin]);
        ...
        for (const item of EQUIPOS_INICIALES) {
            try {
                ...
                const [filasPais] = await conexion.query("SELECT id FROM countries WHERE name = ?", [item.pais]);
                const idPais = filasPais[0]?.id;
                if (!idPais)
                    continue;
                // Obtenemos el ID de la liga
                const [filasLiga] = await conexion.query("SELECT id FROM leagues WHERE name = ? AND country_id = ?", [item.liga, idPais]);
                const idLiga = filasLiga[0]?.id;
                if (!idLiga)
                    continue;
                // Insertamos los equipos
                for (const nombreEquipo of item.equipos) {
                    await conexion.query("INSERT IGNORE INTO teams (name, league_id, country_id) VALUES (?, ?, ?)", [nombreEquipo, idLiga, idPais]).catch(() => { });
                }
            }
            catch (err) {
