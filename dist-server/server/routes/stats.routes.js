 ...
 */
function obtenerUsuarioOpcional(req) {
 ...
 */
function puedeVerDetallePendienteVip(pick, usuario, planesActivos) {
 ...
 */
function parseSeleccionesStats(rawSelections) {
 ...
 */
function resolverEstadoSeleccionStats(selection, marketLabel, marketAcronym) {
    ...
    const marketReference = String(marketLabel ||
        marketAcronym ||
 ...
 */
function enriquecerSeleccionesStats(rawSelections, leagueMap, marketMap) {
 ...
 */
function ocultarDetalleVipPendiente(pick) {
 ...
 */
function prepararPickStats(pick, usuario, planesActivos, leagueMap, marketMap) {
...
router.get("/performance", async (req, res) => {
    try {
        ...
        const [picks] = await pool.query(`SELECT p.status, p.stake, p.odds, pt.slug AS pick_type_slug 
       FROM picks p 
       LEFT JOIN pick_types pt ON p.pick_type_id = pt.id 
       WHERE p.status IN ('won', 'lost', 'void') ${filtroFecha}`, parametros);
...
router.get("/monthly-yield", async (_req, res) => {
    ...
    `);
        ...
        const filasPorMes = new Map(filas.map((fila) => [String(fila.mes), fila]));
...
router.get("/revenue", authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        const [ingresosPorDia] = await pool.query(`SELECT 
       ...
       ORDER BY fecha ASC`, parametros);
        // Distribución de planes activos (instantánea del momento actual)
        const [distribucionPlanes] = await pool.query(`
    ...
    `);
        ...
        const [totalPorPlan] = await pool.query(`SELECT 
         plan_id,
         SUM(amount) AS total_cop,
         SUM(amount_usd) AS total_usd
       FROM user_subscriptions
       ${filtroFecha}
       GROUP BY plan_id`, parametros);
        return res.json({
            revenueByDay: ingresosPorDia.map((r) => ({ ...r, date: r.fecha, plan_id: r.plan_id })),
            planDistribution: distribucionPlanes.map((r) => ({ plan_id: r.plan_id, count: Number(r.cantidad) })),
            totalRevenue: totalPorPlan.map((r) => ({ plan_id: r.plan_id, total_cop: Number(r.total_cop), total_usd: Number(r.total_usd) })),
        });
    }
    catch (error) {
...
router.get("/advanced", authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        const [porLiga] = await pool.query(`SELECT 
         l.name AS liga,
       ...
       LIMIT 15`, parametros);
        ...
        const [porMercado] = await pool.query(`SELECT 
         p.pick AS mercado,
         COUNT(*) AS total_picks,
         SUM(CASE WHEN p.status = 'won'  THEN 1 ELSE 0 END) AS ganados,
         SUM(CASE WHEN p.status = 'lost' THEN 1 ELSE 0 END) AS perdidos,
...
router.get("/historical-picks", async (req, res) => {
    try {
       ...
       LIMIT ? OFFSET ?`, [...parametros, parseInt(String(limit), 10), parseInt(String(offset), 10)]);
        // Consultamos ligas con país para enriquecer selecciones de parlays.
        const [ligas] = await pool.query(`SELECT l.id, l.name, c.name AS country_name, c.flag AS country_flag
       FROM leagues l
       LEFT JOIN countries c ON l.country_id = c.id`);
        ...
        const leagueMap = new Map(ligas.map((league) => [String(league.id), league]));
        // Indexamos mercados por ID para resolver selecciones sin consultas por fila.
        const marketMap = new Map(mercados.map((market) => [String(market.id), market]));
        // Aplicamos reglas de visibilidad VIP antes de responder al frontend.
        const picksSeguros = picks.map((pick) => prepararPickStats(pick, usuario, planesActivos, leagueMap, marketMap));
        // Contamos el total para la paginación en el frontend
        const [total] = await pool.query(`SELECT COUNT(*) AS total FROM picks p
       LEFT JOIN pick_types pt ON p.pick_type_id = pt.id
       WHERE ${whereSql}`, parametros);
        return res.json({
