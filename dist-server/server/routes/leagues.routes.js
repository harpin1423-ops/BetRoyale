...
countriesRouter.post("/", authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const [existentes] = await pool.query("SELECT id FROM countries WHERE name = ?", [name]);
        if (existentes.length > 0) {
            return res.status(400).json({ error: "El país ya existe" });
        }
        // Insertamos el nuevo país (flag es opcional)
        const [resultado] = await pool.query("INSERT INTO countries (name, flag) VALUES (?, ?)", [name, flag || null]);
...
countriesRouter.put("/:id", authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const [existentes] = await pool.query("SELECT id FROM countries WHERE name = ? AND id != ?", [name, id]);
        if (existentes.length > 0) {
...
leaguesRouter.post("/", authenticateToken, requireAdmin, async (req, res) => {
    ...
    const countryIdFinal = country_id && country_id !== "" ? parseInt(country_id.toString(), 10) : null;
    if (!name)
        return res.status(400).json({ error: "El nombre es obligatorio" });
    try {
        ...
        const [resultado] = await pool.query("INSERT INTO leagues (name, country_id) VALUES (?, ?)", [name, countryIdFinal]);
        return res.status(201).json({
            id: resultado.insertId,
            message: "Liga creada",
        });
...
leaguesRouter.put("/:id", authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        await pool.query("UPDATE leagues SET name = ?, country_id = ? WHERE id = ?", [name, countryIdFinal, id]);
