 ...
 */
async function generarLinksVipPrivados(userId, planesActivos, forceRefresh = false) {
    ...
    const [filasVip] = await pool.query(query, slugs);
...
router.get("/profile", authenticateToken, async (req, res) => {
    try {
        ...
        const [filas] = await pool.query(`SELECT id, email, role, vip_until, plan_type, vip_since, 
       ...
       FROM user_subscriptions WHERE user_id = ? AND expires_at > NOW()`, [req.user.id]);
 ...
 */
router.put("/bankroll", authenticateToken, async (req, res) => {
...
router.get("/telegram-links", async (req, res) => {
    try {
        ...
        if (userId) {
            ...
            const [planesActivosRows] = await pool.query(`SELECT DISTINCT plan_id
         FROM user_subscriptions
         WHERE user_id = ? AND expires_at > NOW()`, [userId]);
            // Convertimos las suscripciones en una lista simple de slugs.
            const planesActivos = planesActivosRows.map((s) => s.plan_id);
        ...
        const [filasFree] = await pool.query("SELECT telegram_invite_link FROM pick_types WHERE slug = 'free' LIMIT 1");
        // El enlace gratuito queda disponible para visitantes y usuarios sin VIP.
        const enlaceGratuito = filasFree[0]?.telegram_invite_link || env.TELEGRAM_FREE_INVITE_LINK || "#";
        return res.json({
            /** Enlace al canal gratuito de Telegram */
            free: enlaceGratuito,
            /** Lista de canales VIP a los que tiene acceso */
            vip: canalesVip.map((s) => ({
                name: s.name,
 ...
 */
router.get("/plan-settings", authenticateToken, async (req, res) => {
    try {
        const [filas] = await pool.query("SELECT pick_type_id, initial_bankroll FROM user_plan_settings WHERE user_id = ?", [req.user.id]);
 ...
 */
router.put("/plan-settings", authenticateToken, async (req, res) => {
    const { pick_type_id, initial_bankroll } = req.body;
    try {
        // Usamos UPSERT (INSERT ... ON DUPLICATE KEY UPDATE) para crear o actualizar
        await pool.query(`INSERT INTO user_plan_settings (user_id, pick_type_id, initial_bankroll) 
       VALUES (?, ?, ?) 
       ON DUPLICATE KEY UPDATE initial_bankroll = VALUES(initial_bankroll)`, [req.user.id, pick_type_id, initial_bankroll]);
...
router.get("/metrics", authenticateToken, async (req, res) => {
    try {
        ...
        const [filaUsuario] = await pool.query("SELECT vip_since, initial_bankroll FROM users WHERE id = ?", [req.user.id]);
        const usuario = filaUsuario[0];
        ...
        const [picks] = await pool.query(`SELECT match_date AS date, status, odds, stake 
       FROM picks 
       WHERE pick_type_id != 1 
         AND match_date >= ? 
         AND status IN ('won', 'lost', 'half-won', 'half-lost', 'void')
       ORDER BY match_date ASC`, [usuario.vip_since]);
        // Variables para el cálculo acumulado de métricas
        const listaPicks = picks;
...
router.put("/:id/vip", authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        await pool.query("UPDATE users SET role = 'vip', vip_until = ? WHERE id = ?", [fechaFormateada, id]);
