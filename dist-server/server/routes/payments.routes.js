...
import { createMercadoPagoPreference, getMercadoPagoPayment, obtenerTasaCambio, activarSuscripcion, } from "../services/mercadopago.service.js";
...
router.post("/mercadopago", authenticateToken, async (req, res) => {
    const { title, // Título del item a mostrar en MP
    ...
    try {
        ...
        if (precioCOP <= 0) {
            ...
            const [filaUsuario] = await pool.query("SELECT email, vip_until FROM users WHERE id = ?", [req.user.id]);
            if (filaUsuario[0]) {
        ...
        const referenciaExterna = `sub:${req.user.id}:${planId}:${period}${promoCode ? `:${promoCode}` : ""}`;
        ...
        const esErrorAutorizacionMp = error?.status === 401 ||
            error?.status === 403 ||
...
router.get("/sync", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if ((pago.status === "approved" || pago.status === "authorized") &&
            pago.external_reference) {
            const refExterna = typeof pago.external_reference === "string" ? pago.external_reference : "";
            ...
            const [filaUsuario] = await pool.query("SELECT email, vip_until FROM users WHERE id = ?", [userId]);
            // Enviamos email de confirmación de forma no bloqueante
            if (filaUsuario[0]) {
                const hasta = new Date(filaUsuario[0].vip_until).toLocaleDateString("es-ES");
                enviarEmailConfirmacionVIP(filaUsuario[0].email, planId, hasta).catch((err) => console.error("[PAYMENTS] Error enviando email confirmación:", err));
            }
...
router.post("/webhook", async (req, res) => {
    ...
    try {
        ...
        await pool.query("INSERT INTO webhook_logs (topic, resource_id, raw_body) VALUES (?, ?, ?)", [topic || "unknown", String(id), JSON.stringify({ query: req.query, body: req.body })]).catch((err) => console.error("[WEBHOOK] Error guardando log:", err));
