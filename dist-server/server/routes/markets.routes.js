...
router.post("/", authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const [existentes] = await pool.query("SELECT id FROM markets WHERE id = ?", [id]);
...
router.put("/:id", authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        if (nuevoId !== id) {
            const [existentes] = await pool.query("SELECT id FROM markets WHERE id = ?", [nuevoId]);
            if (existentes.length > 0) {
                return res.status(400).json({ error: "Ya existe un mercado con ese acrónimo" });
            }
        }
        // Actualizamos tanto el ID como el label y acronym
        await pool.query("UPDATE markets SET id = ?, label = ?, acronym = ? WHERE id = ?", [nuevoId, label, acronym, id]);
